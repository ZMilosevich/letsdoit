...
const hr: Record<string, string> = {
  ...
  'Here’s how your clients are moving this week.':'Ovako napreduju vaši klijenti ovaj tjedan.','Add client':'Dodaj klijenta',
  ...
  'Weekly adjustment ready':'Tjedna prilagodba je spremna','Review plan':'Pregledaj plan','Training status and weekly momentum':'Status treninga i tjedni napredak','Search clients':'Pretraži klijente','Filter clients':'Filtriraj klijente','All clients':'Svi klijenti',
  ...
  'Brace before each rep. Keep pressure through the whole foot.':'Učvrstite trup prije svakog ponavljanja. Zadržite oslonac cijelim stopalom.','Hinge at the hips and keep the load close.':'Pregib započnite iz kukova i držite opterećenje blizu tijela.','Use a range that stays pain free.':'Koristite opseg pokreta bez boli.','Keep shoulder blades gently set throughout.':'Tijekom cijelog pokreta lagano držite lopatice stabilnima.','Lead with the elbow without rotating.':'Vodite laktom bez rotacije trupa.','Stack ribs over hips and move smoothly.':'Rebra držite iznad kukova i krećite se kontrolirano.','Drive the floor away and finish tall.':'Gurajte podlogu od sebe i završite uspravno.','Keep a straight line from shoulders to heels.':'Zadržite ravnu liniju od ramena do peta.','Keep output repeatable across all intervals.':'Održavajte ujednačen intenzitet kroz sve intervale.','Stay at an easy, sustainable pace.':'Održavajte lagani, održivi tempo.','Move slowly and avoid pinching or pain.':'Krećite se polako i izbjegavajte nelagodu ili bol.',
};
...
const canonicalEnglish: Record<string,string> = { 'Snaga gornjeg dijela tijela':'Upper body build' };
...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  const t = (value: string) => language === 'hr' ? translateCroatian(value) : canonicalEnglish[value] || Object.entries(hr).find(([, translated]) => translated === value)?.[0] || value;
